...
export default function InventoryPage() {
  ...
  const { data: inventoryData, isLoading } = useQuery({
    ...
    queryFn: () => storeId ? api.inventory.getByStore(storeId) : api.inventory.getAll(),
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold tracking-tight text-slate-900">Inventory Status</h2>
          <p className="text-sm text-slate-500">Monitor stock levels across branches.</p>
        </div>
        <div className="flex items-center gap-2">
          {isAdmin && (
            <Button variant="outline" onClick={() => refreshViewMutation.mutate()} disabled={refreshViewMutation.isPending}>
              <RefreshCw className={`mr-2 h-4 w-4 ${refreshViewMutation.isPending ? 'animate-spin' : ''}`} />
      ...
      <div className="flex items-center gap-2">
        <div className="relative flex-1 max-w-md">
          ...
          <Input placeholder="Search inventory by product name or ID..." className="pl-9 bg-white" />
        </div>
      </div>
        ...
        <Table>
          <TableHeader className="bg-slate-50">
            <TableRow>
              ...
              {isAdmin && <TableHead className="font-semibold text-slate-600">Store</TableHead>}
            ...
            {isLoading ? (
              ...
              inventory.map((item: any) => (
                <TableRow key={item.id} className="hover:bg-slate-50/50">
                  <TableCell className="font-medium text-slate-900">Product {item.productId}</TableCell>
                  {isAdmin && <TableCell>Store {item.storeId}</TableCell>}
                  <TableCell className="font-bold">{item.quantity}</TableCell>
                  <TableCell>
                    <Badge variant="outline" className={`font-semibold border-transparent ${getStatusColor(item.status || 'In Stock')}`}>
                       {item.status || (item.quantity > 10 ? 'In Stock' : item.quantity > 0 ? 'Low Stock' : 'Out of Stock')}
                    </Badge>
                  </TableCell>
                  <TableCell className="text-slate-500">
                    {item.updatedAt ? format(new Date(item.updatedAt), 'PP') : 'N/A'}
                  </TableCell>
