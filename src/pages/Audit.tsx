...
export default function AuditPage() {
  ...
  const { data: auditData, isLoading: isLoadingLogs } = useQuery({
    ...
    queryFn: () => storeId ? api.audit.getByStore(storeId, { page, limit: 50 }) : api.audit.getAll({ page, limit: 50 }),
  });
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 shrink-0">
        <Card className="bg-white border-slate-200">
          <CardContent className="p-4">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest text-center">Total Writes</p>
            <h3 className="text-2xl font-bold text-slate-900 text-center mt-1">{summary.inserts + summary.updates}</h3>
          </CardContent>
        </Card>
        <Card className="bg-white border-slate-200">
          <CardContent className="p-4">
            <p className="text-xs font-bold text-slate-500 uppercase tracking-widest text-center">Deletions</p>
            <h3 className="text-2xl font-bold text-slate-900 text-center mt-1">{summary.deletes}</h3>
          </CardContent>
        </Card>
        <Card className="bg-purple-50/50 border-purple-200 shadow-sm md:col-span-2">
          <CardContent className="p-4 flex items-center justify-between">
            <div>
              <p className="text-xs font-bold text-purple-600 uppercase tracking-widest">Triggers fired this month</p>
              <h3 className="text-3xl font-bold text-purple-900 mt-1">{summary.triggers}</h3>
            </div>
